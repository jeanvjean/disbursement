    ...
    async msg => {
      try {
        ...
        switch (`${message.action}_${message.type}`) {
          case 'send_exported_audit_logs_fire':
            {
              console.log(' [Received] %s', message.type);
              await sendExportedAuditLogs(message.data);
              console.log(
                ' [Processed] %s',
                `SendExportedAuditLogsWorker - ${message.type}`
              );
        ...
        console.error({ RetrySendingAuditLogsExportError: error });
        channel.nack(msg);
      }
    },
