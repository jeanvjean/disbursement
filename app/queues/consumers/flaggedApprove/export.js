...
const { exportFlaggedApprove } = require('../../../utils/flaggedApprove');
      ...
      async msg => {
        ...
        try {
          switch (`${message.action}_${message.type}`) {
            case 'exports_flagged_approve_fire':
              {
                console.log(' [Received] %s', message.action);
                await exportFlaggedApprove(message.data);
                console.log(
                  ' [Processed] %s',
                  `ExportsFlaggedApproveWorker - ${message.action}`
                );
          ...
          console.error({ ExportsFlaggedApproveWorker: error });
