...
const { sendExportedFlaggedApprove } = require('../../../utils/flaggedApprove');

const run = async () => {
  const { channel } = await rabbitmq();
  const { queue, exchange, routingKey } = await rabbitmqArchitecture(
    'send_exported_flagged_approve_queue'
  );
    ...
    async msg => {
      try {
        ...
        switch (`${message.action}_${message.type}`) {
          case 'send_exported_flagged_approve_fire':
            {
              console.log(' [Received] %s', message.type);
              await sendExportedFlaggedApprove(message.data);
              console.log(
                ' [Processed] %s',
                `SendExportedFlaggedApproveWorker - ${message.type}`
              );
        ...
        console.error({ RetrySendingExportedFlaggedApproveExportError: error });
        channel.nack(msg);
      }
    },
